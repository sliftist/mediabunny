...
type AviSample = {
	offset: number; // absolute file offset of the packet's data
	size: number;
	key: boolean;
	timestamp: number;
	duration: number;
...
export class AviDemuxer extends Demuxer {
	...
	private async slice(pos: number, len: number) {
	...
	private async parseHdrl(start: number, size: number) {
		let pos = start;
		const end = start + size;
		let currentStream: AviStream | null = null;
	...
	private async parseStrl(start: number, size: number, stream: AviStream) {
		let pos = start;
		const end = start + size;
		while (pos + 8 <= end) {
			const hdr = await this.slice(pos, 8);
	...
	private async parseIdx1(idx1Pos: number, idx1Size: number, moviDataStart: number) {
	...
	private async scanMovi(moviDataStart: number, fileSize: number) {
	...
	private assignTimestamps() {
		// Video first: exact timestamps straight off the frame clock.
		for (const stream of this.streams) {
			if (stream.type === 'video') {
		...
		// Audio is timed off the video timeline by interleaving when we have a
		// usable video track; otherwise off a byte clock.
	...
	private assignInterleavedAudioTimestamps(audio: AviStream, video: AviStream) {
		...
		// Muxers interleave audio a fraction of a second *ahead* of the video it
		// plays with (decoder preload), so the offset mapping above lands every
		// chunk a constant ~preload early. The stream header gives the exact total
		// audio duration, so the gap between it and where the last chunk mapped is
		// that preload — shift the whole track forward by it to cancel the constant
		// lag. Guarded to a sane range so a long audio tail (which would extrapolate
		// past the last video frame) or a missing header duration just leaves the
		// uncorrected mapping, which is still drift-free to within a frame interval.
		const headerDur = audio.length > 0 && audio.scale > 0 && audio.rate > 0
			? audio.length * audio.scale / audio.rate
			: 0;
		...
		// Each chunk runs until the next one starts; the last extends to the exact
		// total audio duration from the stream header (else the prior chunk's span).
		for (let i = 0; i < n; i++) {
	...
	private assignByteClockAudioTimestamps(stream: AviStream) {
		let totalBytes = 0;
		for (const sample of stream.samples) {
			totalBytes += sample.size;
		}
		const headerDur = stream.length > 0 && stream.scale > 0 && stream.rate > 0
			? stream.length * stream.scale / stream.rate
			: 0;
	...
	async readSampleData(sample: AviSample, options: PacketRetrievalOptions): Promise<Uint8Array> {
...
const makePacket = (
	...
	return demuxer.readSampleData(sample, options).then(data => new EncodedPacket(
		data,
...
const packetIndexForTimestamp = (stream: AviStream, timestamp: number) => {
...
class AviVideoTrackBacking implements InputVideoTrackBacking {
	constructor(public demuxer: AviDemuxer, public stream: AviStream) {}
	...
	getRotation() { return 0 as Rotation; }
	async getColorSpace(): Promise<VideoColorSpaceInit> { return {}; }
	...
	async getKeyPacket(timestamp: number, options: PacketRetrievalOptions) {
		...
		while (i >= 0 && !this.stream.samples[i]!.key) { i--; }
		if (i < 0) { return null; }
		return makePacket(this.demuxer, this.stream, i, options);
	}
	...
	async getNextKeyPacket(packet: EncodedPacket, options: PacketRetrievalOptions) {
		...
		while (i < this.stream.samples.length && !this.stream.samples[i]!.key) { i++; }
		if (i >= this.stream.samples.length) { return null; }
		return makePacket(this.demuxer, this.stream, i, options);
	}
}

class AviAudioTrackBacking implements InputAudioTrackBacking {
	constructor(public demuxer: AviDemuxer, public stream: AviStream) {}
